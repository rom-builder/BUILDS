...
github_release() {
  ...
  for file in $(ls -A $RELEASE_OUT_DIR | grep -E "$pattern"); do
    ...
    file_release=$(curl -X POST -H "Authorization: Bearer $token" -H "Content-Type: application/octet-stream" -T "$RELEASE_OUT_DIR/$file" "https://uploads.github.com/repos/$repo/releases/$release_id/assets?name=$filename" --output -)
